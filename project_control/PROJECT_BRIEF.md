...
G1 已采用双轨策略 C，并完成九类缺口的公开网络尽调。确认没有覆盖全域的正式公开 GIS/CAD 底包，但存在大钟寺局部官方附件、官方文字约束、开放公服名录及可标注的开放现状底图。当前安全暂停在 `G1-BASELINE-DEPTH-001`，等待选择公开基线深度；不开展正式城市设计或投稿方案。
...
5. 关键方向、范围、法定口径、数据许可、外部联系或凭证访问必须暂停并由人类决定；当前门为 `G1-BASELINE-DEPTH-001`。
...
- `G1-DIR-001=C` 已冻结；公开网络尽调确认应在“分层开放基线、官方碎片优先、先公开询问”之间选择。`G1-BASELINE-DEPTH-001` 等待人类决定，`DESIGN-START` 继续保持 `BLOCKED`。
